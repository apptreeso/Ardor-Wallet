	...
	NRS = (function(NRS, $, undefined) {
		...
		$(".add_message").on("change", function() {
			...
			if ($(this).is(":checked")) {
				...
				if ($recipient_container.length == 0 || $recipient_container.is(":hidden")) {
					...
					$optionalMessage.find('input[name=encrypt_message]').first().prop('checked', false);
					$optionalMessage.find('input[name=encrypt_message]').last().prop('checked', true);
				} else {
		...
		$(".do_not_sign").on("change", function() {
			...
			if ($(this).is(":checked")) {
				...
				if ((!NRS.accountInfo || !NRS.accountInfo.publicKey) && NRS.publicKey === "") {
		...
		modal.on("show.bs.modal", function() {
			...
			let $passphraseWarning = $(this).find(".passphrase_warning");
			let $shufflingNodeRunningWarningText = $(this).find(".shuffling-node-running-warning");
			if (NRS.isPrivateKeyStoredOnHardware()) {
				$(this).find(".secret_phrase").hide();
				$(this).find(".show_secret_phrase").hide();
				$(this).find(".hide_secret_phrase").hide();
				if ($passphraseWarning.length === 0 && $shufflingNodeRunningWarningText.length === 0 || $(this).find("#unsigned_transaction_bytes").length !== 0) {
					$(this).closest("form").find(".secret_phrase input").prop("disabled", true);
					$(this).find(".hardware-wallet").show();
					$(this).find(".hardware-status").html($.t("hardware_wallet_waiting_for_commands"));
					$(this).find(".btn-primary").prop("disabled", false);
				} else {
					$passphraseWarning.find(".passphrase_warning_text").html($.t("passphrase_hardware_wallet_warning"));
					$passphraseWarning.show();
					if ($shufflingNodeRunningWarningText.length === 1) {
						$shufflingNodeRunningWarningText.html($.t("passphrase_hardware_wallet_warning"));
					}
					$(this).find(".hardware-wallet").hide();
					$(this).find(".btn-primary").prop("disabled", true);
				}
			} else {
				$passphraseWarning.find(".passphrase_warning_text").html($.t("passphrase_remote_warning"));
				$(this).find(".hardware-wallet").hide();
				if ($shufflingNodeRunningWarningText.length === 1) {
					if (NRS.isPrivateKeyStoredOnHardware()) {
						$(this).find(".btn-primary").prop("disabled", true);
						$shufflingNodeRunningWarningText.html($.t("shuffling_hd_wallet_warning"));
					} else {
						$(this).find(".btn-primary").prop("disabled", false);
						$shufflingNodeRunningWarningText.html($.t("shuffling_node_running_warning",
            ...
            if (NRS.isParentChain()) {
                $(this).find(".fee-calculation-container").css("display", "inline-block");
                $(this).find(".fee-priority-element").hide();
            } else {
                $(this).find(".fee-calculation-container").css("display", "");
                $(this).find(".fee-priority-element").show();
                NRS.updateFeePriorityText($(this).find(".fee-priority-slider"));
            }
		});
		...
		modal.on("hidden.bs.modal", function() {
			...
			$(this).find(".callout-danger:not(.never_hide, .passphrase_warning), .error_message, .account_info").html("").hide();
        ...
        var priorityTextPerIndex = ["transaction_priority_low", "transaction_priority_normal", "transaction_priority_high"]
        NRS.updateFeePriorityText = function($slider) {
