...
NRS.onSiteBuildDone().then(() => {
    NRS = (function(NRS) {
            ...
            NRS.sendRequest("getSupportedContracts", params,
                function(response) {
                    ...
                    var header = {
                        ...
                        "hasPrivateKey": response.hasPrivateKey ? $.t('yes') : $.t('no'),
                        "isValidator": response.isValidator ? $.t('yes') : $.t('no'),
                        "hasValidatorPrivateKey": response.hasValidatorPrivateKey ? $.t('yes') : $.t('no'),
                        "hasRandomSeed": response.hasRandomSeed ? $.t('yes') : $.t('no'),
                        "autoFeeRateFormatted": formatAutoFeeRate(response.autoFeeRate, response.autoFeeRatePriority)
                    };
        ...
        function formatAutoFeeRate(autoFeeRate, autoFeeRatePriority) {
            if (autoFeeRate) {
                const prefix = $.t('yes') + ', ' + $.t('transaction_priority') + ' ';
                switch(autoFeeRatePriority) {
                    case 'LOW' : return prefix + $.t('transaction_priority_low');
                    case 'NORMAL' : return prefix + $.t('transaction_priority_normal');
                    case 'HIGH' : return prefix + $.t('transaction_priority_high');
                    default : return prefix + $.t('unknown');
                }
            } else {
                return $.t('no');
            }
