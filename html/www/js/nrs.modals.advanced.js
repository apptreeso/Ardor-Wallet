...
NRS.onSiteBuildDone().then(() => {
    NRS = (function(NRS, $, undefined) {
        var sessionPrivateKey;

        NRS.setAdvancedModalPrivateKey = function(privateKey) {
            sessionPrivateKey = privateKey;
        };

        NRS.showRawTransactionModal = function(transaction) {
            let $hardwareWalletContainer = $("#raw_transaction_modal_hardware_wallet_container");
            let $hardwareWalletLink = $(".hardware_wallet_sign_tx_link");
            if (NRS.isHardwareTransactionSigningEnabled()) {
                $hardwareWalletContainer.show();
                $hardwareWalletLink.show();
            } else {
                $hardwareWalletContainer.hide();
                $hardwareWalletLink.hide();
            }
            ...
            if (transaction.transactionJSON) {
                ...
                NRS.prepareTextDownloadLink(downloadLink, jsonStr, `${namePrefix}.transaction.${transaction.transactionJSON.timestamp}.json`);
            }
        ...
        NRS.showVoucherModal = function (transaction, signature, publicKey, requestType) {
            ...
            NRS.prepareTextDownloadLink(downloadLink, jsonStr, `voucher.${Date.now()}.json`);
        ...
        NRS.forms.loadVoucher = async function($modal, $btn) {
            ...
            if ($btn.attr("id") == "voucher_submit_btn") {
                let signature;
                if (NRS.isHardwareTransactionSigningEnabled()) {
                    signature = await NRS.signBytesUsingHardwareWallet(voucherJson.unsignedTransactionBytes);
                    if (signature === null) {
                        rc.error = $.t("hardware_wallet_txn_signing_cancelled");
                        return rc;
                    }
                } else {
                    let privateKey;
                    if (sessionPrivateKey) {
                        ...
                        rc.error = $.t("cannot_sign_voucher");
                        return rc;
                    }
                    signature = await NRS.signBytesWrapper(voucherJson.unsignedTransactionBytes, privateKey);
                }
                let signedTransactionBytes = NRS.insertSignature(voucherJson.unsignedTransactionBytes, signature);
                NRS.broadcastTransactionBytes(signedTransactionBytes, function (broadcastResponse) {
                    if (NRS.isErrorResponse(broadcastResponse)) {
                        var error = NRS.getErrorMessage(broadcastResponse);
                        $.growl(error, {
                            "type": "danger"
                        });
                        NRS.logConsole(error);
                    } else if (broadcastResponse.fullHash) {
                        NRS.addUnconfirmedTransaction(broadcastResponse.fullHash, function () {
                            NRS.forms.loadVoucherComplete();
                        });
                    }
                }, {}, {}, transactionJSON.attachment);
                rc.stop = true;
            } else if ($btn.hasClass("btn-calculate-fee")) {
                let calculateFeeResponse = await NRS.sendRequestAndWait("calculateFee", {
        ...
        $(".hardware_wallet_sign_tx_link").click(function(e) {
            e.preventDefault();
            new Promise(function(resolve) {
                var signature = NRS.signBytesUsingHardwareWallet($("#raw_transaction_modal_unsigned_transaction_bytes").val());
                resolve(signature);
            }).then(function(signature) {
                if (signature === null) {
                    $.growl($.t("transaction_signing_cancelled"));
                    $("#raw_transaction_modal_signature").val("");
                } else {
                    $("#raw_transaction_modal_signature").val(signature);
                }
            })
        });
        ...
        transactionJSONModal.on("show.bs.modal", function(e) {
            ...
            if (NRS.isHardwareTransactionSigningEnabled() || !NRS.accountRS) {
        ...
        NRS.forms.signTransactionComplete = function(response) {
            ...
            NRS.prepareTextDownloadLink(downloadLink, jsonStr, `signed.transaction.${response.transactionJSON.timestamp}.json`);
        ...
        NRS.forms.signTransaction = function() {
            ...
            if ((data.unsignedTransactionBytes && !data.validate) || NRS.isHardwareTransactionSigningEnabled()) {
                let output = {};
                output.stop = true;
                output.keepOpen = true;
                let accountId;
                let privateKey;
                if (sessionPrivateKey || data.secretPhrase) {
                    privateKey = (NRS.rememberPassword ? sessionPrivateKey : NRS.getPrivateKey(data.secretPhrase));
                    accountId = NRS.getAccountId(privateKey);
                }
                if (!NRS.account || !accountId || accountId === NRS.account) {
                    try {
                        let signingPromise = NRS.getSigningPromise(data.unsignedTransactionBytes, privateKey);
                    ...
                    output.errorMessage = $.t("error_passphrase_incorrect_v2", {account: NRS.convertNumericToRSAccountFormat(accountId) });
                }
                return output;
            } else {
