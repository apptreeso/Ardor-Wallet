...
NRS.onSiteBuildDone().then(() => {
    NRS = (function(NRS, $) {
        ...
        $('#configuration_restart_required button').on('click', function () {
            const texts = ['<p>' + $.t('save_and_shutdown_confirmation') + '</p>'];
            const properties = configProperties.filter(p => p.isWithNewValue)
                .map(p => ({property: p.name, value: p.newValue}));
            const updatedProperties = properties.filter(p => p.value !== '');
            const removedProperties = properties.filter(p => p.value === '');
            if (updatedProperties.length > 0) {
                texts.push('<p class="lead">' + $.t('updated_properties') + '</p>');
                texts.push('<dl>');
                updatedProperties.forEach(p => {
                    texts.push(`<dt>${String(p.property).escapeHTML()}</dt>`);
                    texts.push(`<dl>${String(p.value).escapeHTML()}</dl>`);
                });
                texts.push('</dl>');
            }
            if (removedProperties.length > 0) {
                ...
                removedProperties.forEach(p => {
                    texts.push(`<li>${String(p.property).escapeHTML()}</li>`);
                });
                texts.push('</ul>');
            }
