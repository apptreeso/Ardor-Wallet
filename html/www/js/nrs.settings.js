...
NRS.onSiteBuildDone().then(() => {
	NRS = (function(NRS, $) {
		...
		function countryFromLanguage(code) {
			switch(code) {
				case 'en':
					return 'gb';
				case 'el':
		...
		NRS.createLangSelect = function() {
			const $langSelectors = $('.languageSelector ul');
			$langSelectors.empty();
			$.each(NRS.languages, function(code, name) {
				const $li = $('<li></li>');
				$li.data('language', code);
				$li.html(`<a href="#"><span class="flag-icon flag-icon-${(countryFromLanguage(code))}"></span> ${name}</a>`);
				$langSelectors.append($li);
			});
			const currentLanguage = NRS.languages[NRS.settings.language];
			$('.languageSelector button > span.currentLanguage').html(`<span class="flag-icon flag-icon-${countryFromLanguage(NRS.settings.language)}"></span> ${currentLanguage}`);
		};
		...
		NRS.initSettings = function() {
			$("#settings_box select, #settings_admin_password").on("change", function(e) {
				e.preventDefault();
			...
			$('#login_panel .languageSelector, #settings_box .languageSelector').on('click', 'li', function(e) {
				...
				$('.languageSelector button > span.currentLanguage').html($(this).find('a').html());
			});
			...
			$("#settings_box").find("input[type=text]").on("input", function() {
				...
				let isValid = input.reportValidity ? input.reportValidity() : input.checkValidity(); //for standalone wallet which doesn't support reportValidity API
				if (isValid) {
					NRS.updateSettings(key, value);
				} else {
					console.warn(`Invalid setting value entered in the field ${key}: ${value}`);
				}
			});
