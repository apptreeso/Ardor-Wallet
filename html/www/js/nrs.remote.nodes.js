    ...
    NRS = (function(NRS) {
        ...
        NRS.requestNeedsConfirmation = function (requestType) {
            if (NRS.remoteNodesMgr) {
                ...
                return !NRS.isPostRequiredByServer(requestType) && NRS.isRequestForwardable(requestType)
            }
            return false;
        };
        ...
        NRS.confirmResponse = function(requestType, data, expectedResponse, requestRemoteNode) {
            if (NRS.requestNeedsConfirmation(requestType)) {
                ...
                var nodes = NRS.remoteNodesMgr.getRandomNodes(NRS.deviceSettings.validators_count, ignoredAddresses);
                var now = new Date();
