...
NRS.onSiteBuildDone().then(() => {
	NRS = (function(NRS, $) {
		...
		NRS.dgs_search_main = function(callback) {
			...
			$(".dgs_search input[name=q]").val("").trigger("unmask").mask(NRS.getAccountMask("*"));
		...
		NRS.forms.dgsFeedback = async function($modal) {
			let data = NRS.getFormData($modal.find("form:first"));
			let response = await NRS.sendRequestAndWait("getDGSPurchase", {
		...
		NRS.forms.dgsPurchase = async function($modal) {
			let data = NRS.getFormData($modal.find("form:first"));
			let response = await NRS.sendRequestAndWait("getDGSGood", { "goods": data.goods });
			if (response.errorCode) {
				return {
					"error": $.t("error_goods")
				};
		...
		NRS.forms.dgsRefund = async function($modal) {
			let data = NRS.getFormData($modal.find("form:first"));
			let response = await NRS.sendRequestAndWait("getDGSPurchase", {
				"purchase": data.purchase
			});
		...
		NRS.forms.dgsDelivery = async function($modal) {
			var data = NRS.getFormData($modal.find("form:first"));
			let response = await NRS.sendRequestAndWait("getDGSPurchase", {
				"purchase": data.purchase
			});
			...
			try {
				if (data.doNotSign) {
					...
					if (NRS.isHardwareEncryptionEnabled()) {
						...
						// todo: test what happens if we don't have secret phrase?
						encrypted = NRS.encryptNote(data.data, {
							"account": data.buyer,
		...
		NRS.forms.dgsQuantityChange = async function($modal) {
			let data = NRS.getFormData($modal.find("form:first"));
			let response = await NRS.sendRequestAndWait("getDGSGood", { "goods": data.goods });
			if (!response.errorCode) {
		...
		$("#dgs_refund_modal, #dgs_delivery_modal, #dgs_feedback_modal, #dgs_view_purchase_modal, #dgs_view_delivery_modal, #dgs_view_refund_modal").on("show.bs.modal", async function(e) {
			var $modal = $(this);
			...
			if (response.errorCode) {
				...
				let good = await NRS.sendRequestAndWait("getDGSGood", { "goods": response.goods });
				if (good.errorCode) {
					e.preventDefault();
					...
					if (type == "dgs_refund_modal") {
						...
						var refund;
						if (response.discountNQT) {
							refund = orderTotalBeforeDiscount.subtract(new BigInteger(String(response.discountNQT)));
						} else {
							refund = orderTotalBeforeDiscount;
						}
		...
		$("#dgs_product_modal, #dgs_delisting_modal, #dgs_quantity_change_modal, #dgs_price_change_modal, #dgs_purchase_modal").on("show.bs.modal", async function(e) {
			var $modal = $(this);
			var $invoker = $(e.relatedTarget);
			...
			let response = await NRS.sendRequestAndWait("getDGSGood", { "goods": goods });
			if (response.errorCode) {
				e.preventDefault();
				$.growl($.t("error_goods"), {
					"type": "danger"
