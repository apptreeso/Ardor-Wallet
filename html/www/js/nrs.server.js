...
(isNode ? client : NRS).onSiteBuildDone().then(() => {
    NRS = (function (NRS, $, undefined) {
        ...
        function isResponseReadyForSigning(response, data) {
        ...
        NRS.processAjaxRequest = function (requestType, data, callback, options) {
            ...
            } else if (NRS.isPostRequiredByServer(requestType) && NRS.rememberPassword) {
        ...
        function processResponse(data, extra, options, requestType, privateKey, callback, file, isVolatile) {
            return async function (response) {
                if (!options.remoteNode && NRS.isConfirmResponse() &&
                    ...
                    let requestRemoteNode = {address: "localhost", announcedAddress: "localhost"};
                ...
                addAddressData(data);
                if ((privateKey !== undefined && privateKey !== "" || NRS.isHardwareTokenSigningEnabled()) && isResponseReadyForSigning(response, data)) {
            ...
            switch (requestType) {
                ...
                case "dgsDelivery":
                    ...
                    if (goodsIsText != data.goodsIsText) {
                        return {
                            fail: true,
                            param: requestType + "IsText",
                            actual: goodsIsText,
                            expected: data.goodsIsText
                        };
                    }
                ...
                case "issueCurrency":
                    ...
                    if (transaction.minReservePerUnitNQT !== "0" && transaction.minReservePerUnitNQT !== data.minReservePerUnitNQT) {
                        return {
                            fail: true,
                            param: requestType + "MinReservePerUnitNQT",
                            actual: JSON.stringify(data),
                            expected: JSON.stringify(transaction)
                        };
                    }
                    if (transaction.minDifficulty !== "0" && transaction.minDifficulty !== data.minDifficulty) {
                        return {
                            fail: true,
                            param: requestType + "MinDifficulty",
                            actual: JSON.stringify(data),
                            expected: JSON.stringify(transaction)
                        };
                    }
                    if (transaction.maxDifficulty !== "0" && transaction.maxDifficulty !== data.maxDifficulty) {
                        return {
                            fail: true,
                            param: requestType + "MaxDifficulty",
                            actual: JSON.stringify(data),
                            expected: JSON.stringify(transaction)
                        };
                    }
                ...
                case "shufflingCreate":
                    ...
                    if (participantCount !== data.participantCount) {
                        return {
                            ...
                            expected: data.participantCount
                        };
                    }
                    pos++;
                    let registrationPeriod = converters.byteArrayToSignedShort(byteArray, pos);
                    if (registrationPeriod !== data.registrationPeriod) {
                        return {
                            fail: true,
                            param: requestType,
                            actual: registrationPeriod,
                            expected: data.registrationPeriod
                        };
                    }
                    pos += 2;
                    break;
        ...
        NRS.verifyAppendix = function (byteArray, transaction, requestType, data, pos) {
            ...
            if ((transaction.flags & 1) != 0 ||
                ...
                if (messageIsText != data.messageIsText && data.messageIsText !== undefined) {
