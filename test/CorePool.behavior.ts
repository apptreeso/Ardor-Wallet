...
export function updateStakeLock(usingPool: string): () => void {
  return function () {
    it("should update stake lock to two years", async function () {
      ...
      await token.connect(this.signers.alice).approve(pool.address, MaxUint256);
      await pool.connect(this.signers.alice).stakePoolToken(toWei(100), INIT_TIME + 11);
      ...
      await pool.connect(this.signers.alice).updateStakeLock(0, INIT_TIME + 10 + ONE_YEAR);
      ...
      expect(lockedUntil1).to.be.equal(INIT_TIME + 10 + ONE_YEAR);
    ...
    it("should revert if _lockedUntil < _now256", async function () {
      ...
      await token.connect(this.signers.alice).approve(pool.address, MaxUint256);
      await pool.connect(this.signers.alice).stakePoolToken(toWei(100), INIT_TIME);
    ...
    it("should revert if _lockedUntil < previous _lockedUntil", async function () {
      ...
      await expect(pool.connect(this.signers.alice).updateStakeLock(0, INIT_TIME + ONE_YEAR + 21)).reverted;
    });
    it("should revert if locking for more than two years", async function () {
      ...
      await expect(pool.connect(this.signers.alice).updateStakeLock(0, INIT_TIME + ONE_YEAR + 1)).reverted;
    });
  };
}
